...
class UpdatingChannels(Scale):
    ...
    async def bored_channels(self):
        if "nt" not in os.name:  # don't update channels if running as dev bot
            ...
            if total_members.name != f"Total Members: {amount_total_members}":
                await total_members.edit(name=f"Total Members: {amount_total_members}")
            if total_bots.name != f"Total Bots: {amount_total_bots}":
                await total_bots.edit(name=f"Total Bots: {amount_total_bots}")
            if total_guilds.name != f"Total Guilds: {len(self.bot.guilds)}":
                await total_guilds.edit(name=f"Total Guilds: {len(self.bot.guilds)}")
            if guild_members.name != f"Members Here: {bored_guild.member_count}":
