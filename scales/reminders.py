...
async def delete_reminder(ctx: InteractionContext, reminder: str):
...
class Reminders(Scale):
    ...
    async def reminder_add(self, ctx: InteractionContext):
        ...
        client = motor.motor_asyncio.AsyncIOMotorClient(
            (Path(__file__).parent.parent / "mongo.txt").read_text().strip(),
    ...
    async def reminder_list(self, ctx: InteractionContext):
        ...
        try:
            ...
            for reminder in reminders:
                count += 1
                embeds.append(Embed(title=f"<a:reminder:956707969318412348> Reminder {count}", description=f"Content: {reminder['content']}\nDue: <t:{reminder['time']}:F>(<t:{reminder['time']}:R>)",
                                    color=color.FlatUIColors.CARROT))
            ...
            paginator.callback = delete_reminder(ctx, reminder="boop")
            paginator.callback_button_emoji = "❌"
            paginator.show_callback_button = True
            await paginator.send(ctx)
        except IndexError:
            embed = Embed(title="<a:reminder:956707969318412348> You have no reminders", color=color.FlatUIColors.CARROT)
